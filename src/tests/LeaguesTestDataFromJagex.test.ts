...
// http://localhost:3000/osrs-dps?id=RupertsPulleyChaos
  describe('melee', () => {
    test('scy echoes', () => {
      ...
      const p = getTestPlayer(m, {
        equipment: {
          ...
          cape: findEquipment('Infernal cape', 'Normal'),
        ...
        leagues: {
          six: {
            effects: {
              ...
              talent_defence_boost: 5,
              talent_restore_sa_energy_from_distance: 1,
              talent_percentage_melee_damage: 1,
              talent_melee_distance_healing_chance: 10,
      ...
      expect(calc.getDps()).toBeCloseTo(11.08, 0);
      console.log({ scyEchoes: calc.getDps() });
    });

    test('nally blindbag no echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          head: findEquipment('Torva full helm'),
    ...
    test('scythe blindbag no echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          head: findEquipment('Torva full helm'),
  ...
  // http://localhost:3000/osrs-dps?id=DustTrimmedKing
  describe('ranged', () => {
    test('rcb echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
    ...
    test('ddarts echoes', () => {
      ...
      const calc = new PlayerVsNPCCalc(p, m);
      expect(calc.getDps()).toBeCloseTo(7.64, 0);
  ...
  describe('gpc w scy bb', () => {
    test('gpc mainhand', () => {
      ...
      const p = getTestPlayer(m, {
        equipment: {
          weapon: findEquipment('Goblin paint cannon'),
        },
      ...
      const calc = new PlayerVsNPCCalc(p, m);
      expect(calc.getDps()).toBeCloseTo(2.975, 2);
      console.log({ dps: calc.getDps() });
    });

    test('3t scy mainhand', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          weapon: findEquipment('Scythe of vitur', 'Charged'),
        ...
        attackSpeed: 3,
        style: getCombatStylesForCategory(EquipmentCategory.SCYTHE)[0],
        prayers: [],
        leagues: {
          six: {
            effects: {
      ...
      const calc = new PlayerVsNPCCalc(p, m);
      expect(calc.getDps()).toBeCloseTo(10.842, 2);
      console.log({ dps: calc.getDps() });
    });

    test('gpc mainhand with echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          weapon: findEquipment('Goblin paint cannon'),
      ...
      const calc = new PlayerVsNPCCalc(p, m);
      expect(calc.getDps()).toBeCloseTo(3.125, 2);
      console.log({ dps: calc.getDps() });
    });

    test('3t scy mainhand with echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          weapon: findEquipment('Scythe of vitur', 'Charged'),
    ...
    test('gcp w scy bb', () => {
      ...
      const calc = new PlayerVsNPCCalc(p, m);
      expect(calc.getDps()).toBeCloseTo(4.886, 2);
      console.log({ dps: calc.getDps() });
    });

    test('gcp w scy bb and echoes', () => {
      const m = getTestMonster('Gemstone Crab', '', { skills: { def: 0 } });
      const p = getTestPlayer(m, {
        equipment: {
          weapon: findEquipment('Goblin paint cannon'),
