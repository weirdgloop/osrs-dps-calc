...
const MonsterWeaknessElementButton: React.FC<{ element: Spellement | 'none' }> = observer(({ element }) => {
  ...
  const setWeaknessElement = () => {
    if (element === 'none') {
      updateMonsterBase({ weakness: null });
      return;
    }
    if (monsterBase.weakness?.element !== element) {
      updateMonsterBase({ weakness: { element, severity: 0 } });
    }
  };

  const isActive = monsterBase.weakness?.element === element;
  ...
  return (
    ...
    >
      <div className="flex justify-center gap-2 flex-wrap">
        ...
        <MonsterWeaknessElementButton element={Spellement.AIR} />
        <MonsterWeaknessElementButton element={Spellement.WATER} />
        <MonsterWeaknessElementButton element={Spellement.EARTH} />
        <MonsterWeaknessElementButton element={Spellement.FIRE} />
      </div>
      <div className="mt-4 flex flex-col items-center">
        ...
        <div className="flex items-center">
          <p>+</p>
          <NumberInput
            disabled={monsterBase.weakness === null}
            className="form-control w-14 rounded mt-auto"
            onChange={(v) => updateMonsterBase({ weakness: { severity: v } })}
            min={0}
            max={1000}
            value={monsterBase.weakness?.severity ?? 0}
            step={1}
            required
          />
          <p>%</p>
...
const MonsterWeaknessBadge: React.FC = observer(() => {
  ...
  if (!monsterBase.weakness && !isCustomMonster) {
    return null;
  }

  const element = monsterBase.weakness?.element ?? 'none';
  const severity = monsterBase.weakness?.severity ?? 0;
