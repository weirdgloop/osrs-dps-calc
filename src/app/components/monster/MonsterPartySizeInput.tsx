...
const MonsterPartySizeInput: React.FC = observer(() => {
  ...
  if (!PARTY_SIZE_REQUIRED_MONSTER_IDS.includes(monsterBase.id)
    && !monsterBase.attributes.includes(MonsterAttribute.XERICIAN)) {
    return null;
  }
  ...
  return (
    <>
      ...
      <div className="mt-2">
        <NumberInput
          value={monsterBase.inputs.partySize}
          min={1}
          max={100}
          step={1}
          onChange={(v) => updateMonsterBase({ inputs: { partySize: v } })}
          required
        />
      </div>
    </>
